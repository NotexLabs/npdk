...
impl Packer {
    pub fn new<P: AsRef<Path>>(source: P) -> Result<Self> {
        if source.as_ref().exists() {
            return Ok(Self {
                walk_dir: WalkDir::new(source),
            });
        }
        Err(anyhow!(
            format!("{} does not exist", source.as_ref().display().to_string())
                .bright_red()
                .bold()
        ))
    }
    ...
    pub async fn pack(self) -> Result<()> {
        ...
        for entry in self.walk_dir {
            ...
            if path.to_str().unwrap().contains("@mf-types") {
                continue;
            }
            if path.is_file() {
        ...
        if let Some(config) = config {
            ...
            let compress_task = tokio::task::spawn(async move {
                let output_file =
                    File::create(format!("{}.notex.plugin", config.package.name)).await?;
                let mut output = BufWriter::new(output_file.into_std().await);
                let mut compressor = CompressorWriter::new(&mut output, 4096, 11, 22);
            ...
            let pb_task = tokio::task::spawn(async move {
                ...
                pb.set_style(ProgressStyle::default_bar().template(
                    "{spinner:.green} {msg:.white.bold} [{elapsed_precise:.white.bold}]",
                )?);
                ...
                loop {
                    if let Ok(_) = rx.try_recv() {
                        pb.finish_with_message(
                            "Packing plugin completed".bright_green().bold().to_string(),
                        );
                        break;
                    }
                    pb.inc(1);
